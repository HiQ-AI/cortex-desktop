...
### Cowork — 本地优先的 AI 工作台
AI 直接操作你本地的文件。除了 AI 推理和数据库搜索，所有操作都在你的电脑上执行：
- **文件不离本地** — AI 在你的电脑上读取、写入、编辑文件，不会上传
- **本地执行** — Python、Shell 命令、数据处理全部在本地运行
- **本地存储** — 对话记录、项目、工作目录文件全部保存在你的设备上
- 自主规划任务、分配子任务、并行搜索
...
- [x] 项目管理（文件夹、指令、持久记忆）
- [x] 会话管理（星标、归档、重命名、搜索）
- [x] 定时任务 — Cortex 按日/周/cron 自己跑，桌面通知
- [x] 项目任务卡 — 跨会话的待办追踪，挂在每个项目主页
- [x] 知识图谱 — 每个项目自动构建实体/来源/主张图谱，需要时手动 ingest
