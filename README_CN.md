...
### Cowork — 本地优先的 AI 工作台
AI 直接在你的本地文件上干活，自带一整套工具箱：
- **全部本地运行** — 读写编辑文件、执行 Python 与 Shell、对话记录和工作区数据都留在你的电脑上
- **文件上传** — 直接拖入 Excel、CSV、PDF、Word、图片，AI 读取并分析
- **桌面与浏览器自动化** — 需要时让 Cortex 操作其他桌面应用或驱动真实浏览器，按应用授权
...
- **桌面与浏览器自动化** — 让 Cortex 操作其他桌面应用或驱动真实浏览器，按应用授权
- **子代理并行** — 子任务由并行运行的子代理处理，嵌套时间线实时展示每个子任务
- **Artifacts 画廊与场景卡片** — 生成文件的应用内画布，加上常用工作流的快捷入口卡片
- **全面美化** — 新字体、代码块语法高亮、更清爽的表格和输入区（v0.0.73）
